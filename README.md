...
Инструмент достаточно простой в освоении - getting started + `flux <command> --help` с примерами внутри отвечают на все вопросы.
#### Terminology
...
Flagger - инструмент прогрессивной развёртки, автоматизирующий релизный процесс для приложений в Kuber-е. Он снижает риски выкатки новых версий приложения посредством trafic shifting-а, то есть маршрутизации постепенно увеличивающейся доли запросов на новую версию приложения, отслеживая при этом метрики, запуская тесты и выполняя роллбэки при необходимости.

Умеет работать со всеми популярными Service Mesh-ами и стретегиями деплоя (Canary, A/B-testing, Blue-Green, mirroring), собирая при этом метрики из многих видов хранилищ, отправляя уведомления в большинство чатов.

![Flagger Arch](https://raw.githubusercontent.com/fluxcd/flagger/main/docs/diagrams/flagger-overview.png)

#### Canary Resource
CRD этого типа описывает релизный процесс приложения в Kubernetes, при этом которое может применяться на другие кластеры, мешы и ингресс провайдеров.

Например здесь (https://docs.flagger.app/usage/how-it-works#canary-resource), Flagger постепенно смещает трафик на канареечную версию, в то же время измеряя уровень задержек и процент успеха запросов.
Набор метрик можно расширять, добавляя свои собственные.

##### Canary Target
CRD Canary может иметь своей целью(target) либо Deployment либо DaemonSet.
```yaml
spec:
...
Например, на основе конфига выше, Flagger создаёт объекты:
* deployment/<targetRef.name>-primary
* hpa/<autoscalerRef.name>-primary
Primary deployment считается стабильным релизом приложения, по умолчанию весь трафик направляется на эту версию и целевая развёртка масштабируется до 0.
Flagger определит изменения в целевом деплойменте, включая секреты и конфигмапы и выполнит канареечный анализ прежде чем утверждать (to promote) новую версию как primary.
При этом для каждого секрета или конфигмапы так же будет создаваться объект с суффиксом -primary.

Можно использовать `.spec.autoscalerRef.primaryScalerReplicas`, чтобы переписать настройки репликаций для сгенерированного primary HPA.
Важно, чтобы у целевого приложения был только один label формата app=.
Правда есть поддержка и других селекторов.
Директива автомасштабирования опциональна, но когда определена, Flagger остановит увеличение трафика до тех пор, пока целевые(target) и основные(primary) не будут отмасштабированы. Это всё может быть полезно для размазывания нагрузки и снижения отклика.
`progressDeadlineSeconds` - показывает лимит для канареечного деплоя, в противном случае он будет автоматически откачен. По умолчанию 10 минут.

##### Canary Service
Ресурс Canary определяет как целевая нагрузка открыта внутри кластера. Canary target открывает TCP-порт, который будет использован Flagger-ом, чтобы создавать ClusterIP сервисы.
...
spec:
  service:
    ...
    portDiscovery: true
```
Порт контейнера target workload должен совпадать с service.targetPort.
* Service.name - опционально, обычно =spec.targetRef.name
* service.targetPort - либо номер либо имя
* service.portName - опционально, по умолчанию http, но можно и grpc
* service.appProtocol - опционально, больше деталей можно найти [тут](https://kubernetes.io/docs/concepts/services-networking/service/#application-protocol)

Если обнаружение портов включено, то флаггер будет сканировать целевые нагрузки в поисках остальных портов и создавать для них serviceIP.

* `<service.name>.<namespace>.svc.cluster.local` - app=<name>-primary
* `<service.name>-primary.<namespace>.svc.cluster.local` - app=<name>-primary
* `<service.name>-canary.<namespace>.svc.cluster.local` - app=<name>

Это позволит трафик до `podinfo.test:9898` маршрутизироваться до последней стабильной версии нашего приложения. А `podinfo-canary.test:9898`, в свою очередь, будет доступен до тех пор пока будет проходить canary analysis и может использоваться для нагрузочных и тестов соответствия.
...
Promoted - новая версия утверждена как primary.

##### Canary Analysis
Запускается периодически, пока не достигнет максимального веса в трафике или в итерациях.
На каждый запуск Flagger выполняет вебхуки, проверки метрик и, если достигнут порог по проверкам, анализ останавливается и делает откат. А если настроен алертинг, то результаты публикуются вне зависимости от успеха.

Определеяет:
* Тип стратегии деплоя
* Метрики, используемые для подтверждения корректности канареечной версии
* Вебхуки, для тестирования на соответствие, нагрузочного тестирования и ручного стробирования
* Настройки алертов
```yaml
  analysis:
